...
def mask_producer(frame, mask_HSV, color):
    ...
    for c in contours:
        ...
        if r > 35:
            ...
            centres.append((x-320, -y+240, distenc))#(0,0)in der mitte xnach rechts y nach oben
...
def coordinat_zu_ordnen(col, center, detected_Balls):
    ...
    y = center[0, 1]
    d = center[0, 2]
    x_car = x  # d * np.cos(np.deg2rad(alpha / x)) * np.cos(np.deg2rad(betha / y))
    y_car = y  # d * np.sin(np.deg2rad(alpha / x)) * np.sin(np.deg2rad(betha / y))
    """neuer ball"""
    Ball = ObjektBall()
    Ball.color = col
